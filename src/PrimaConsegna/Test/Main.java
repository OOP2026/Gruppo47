...
// ── 1. ENTITY: creazione dati di esempio ─────────────────────────────
        ...
        // Recuperiamo riferimenti agli insegnamenti per creare lezioni di esempio
        ...
        // ── 2. CONTROL: creazione controller BCE ─────────────────────────────

        GestioneOrarioController   gestioneCtrl   = new GestioneOrarioController(orarioGenerale, resp);
        RichiestaController        richiestaCtrl  = new RichiestaController(orarioGenerale, resp);
        VisualizzaOrarioController visualizzaCtrl = new VisualizzaOrarioController(orarioGenerale);

        // ── 3. BOUNDARY: avvio GUI su EDT ────────────────────────────────────

        // Lista docenti per la GestioneRisorsePanel
        ...
        SwingUtilities.invokeLater(() -> {
            // Aspetto grafico del sistema operativo
            try {
                UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
            } catch (Exception ignored) {}

            MainFrame frame = new MainFrame(gestioneCtrl, richiestaCtrl, visualizzaCtrl,
                    docBianchi, studente);

            // Inietta la lista docenti nel pannello risorse
            // (accesso tramite cast; in alternativa si potrebbe usare un metodo pubblico su MainFrame)
            // La lista docenti serve solo al pannello GestioneRisorsePanel
